...
_DARK_THEME = Theme(
    json={
        "attrs": {
            "Plot": {
                "background_fill_color": "#1e1e2e",
                "border_fill_color": "#181825",
                "outline_line_color": "#313244",
            },
            "Grid": {"grid_line_color": "#313244", "grid_line_alpha": 0.7},
            "Axis": {
                "axis_line_color": "#585b70",
                "major_tick_line_color": "#585b70",
                "minor_tick_line_color": "#45475a",
                "major_label_text_color": "#cdd6f4",
                "axis_label_text_color": "#cdd6f4",
            },
            "Title": {"text_color": "#cdd6f4", "text_font_size": "13px"},
            "Legend": {
                "background_fill_color": "#1e1e2e",
                "background_fill_alpha": 0.85,
                "label_text_color": "#cdd6f4",
                "border_line_color": "#45475a",
            },
        }
    }
)
...
_C = ["#89b4fa", "#a6e3a1", "#f38ba8", "#fab387", "#f9e2af", "#cba6f7"]
...
def _make_document(doc: Any, ctx_obj: Any) -> None:
    ...
    p_ang = figure(title="Angles", height=210, x_axis_label="Time [s]", y_axis_label="rad", **kw)
    ...
    p_def = figure(title="Deflection", height=180, x_axis_label="Time [s]", y_axis_label="rad", **kw)
    p_def.line("t", "defl", source=src_def, color=_C[2], line_width=2)

    p_vel = figure(title="Velocity", height=180, x_axis_label="Time [s]", y_axis_label="rad/s", **kw)
    ...
    p_cmd = figure(title="Command vs Measured", height=180, x_axis_label="Time [s]", y_axis_label="rad", **kw)
    ...
    doc.add_root(column(p_ang, p_def, p_vel, p_cmd, sizing_mode="stretch_width"))
    ...
    def update() -> None:
        ...
        src_vel.data = dict(
            t=t,
            mvel=[s.motor_vel_rad_s  for s in samples],
            ovel=[s.output_vel_rad_s for s in samples],
        )
        src_cmd.data = dict(
            t=t,
            cmd=[s.cmd_pos    for s in samples],
            meas=[s.motor_rad for s in samples],
        )
